...
def load_pkl_file(file_path):
    ...
    Parameters:
    file_path (str): The path to load the pickled file (model).
...
def page_machine_learning_body():
    ...
    st.info(
        "* To answer the Business Requirement 2, a Regressor model was trained and the pipeline "
        "tuned aiming for at least 0.9 accuracy in predicting the sales "
        "price of a property with a set of attributes.\n"
        "* The pipeline performance for the best model on the train and test "
        "set is R2 = 1.0 (flawless score) and R2 = 0.883 (almost 0.9) respectively.\n"
    ...
    st.write(
        "* We agreed with the client an R2 average score of at least 0.9 on the "
        "train set and the test set."
    )
    st.write(
        "* Our Machine Learning pipeline performance shows that our model performance "
        "metrics have achieved the requirements, thanks to the flawless score on the train model."
    )
